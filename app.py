...
def validate_file(file):
    filename = file.name
    nae, ext = os.path.splitext(filename)
    if ext in ('.csv', '.xlsx'):
        return ext
    else:
        return False
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        display_mode = st.radio('Display_mode:', options = ('Primary', 'Dark', 'Orange'))
...
if uploaded_file is not None: 
    ...
    if ext:
        ...
        if filesize <= 10:
            ...
            st.error(f"Maximum file size allowed is 10 MB, but reveived {filesize} MB")
    else:
